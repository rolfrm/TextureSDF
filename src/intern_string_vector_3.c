...
intern_string_vector_3 * intern_string_vector_3_create(const char * optional_name){
  ...
  static const char * const column_types[] = {"u256", "u32"};
  intern_string_vector_3 * instance = calloc(sizeof(intern_string_vector_3), 1);
  instance->column_names = (char **)column_names;
  instance->column_types = (char **)column_types;
  
  icy_table_init((icy_table * )instance, optional_name, 2, (unsigned int[]){sizeof(u256), sizeof(u32)}, (char *[]){(char *)"key", (char *)"value"});
...
void intern_string_vector_3_unset(intern_string_vector_3 * table, u256 key){
  intern_string_vector_3_remove(table, &key, 1);
}
...
bool intern_string_vector_3_try_get(intern_string_vector_3 * table, u256 * key, u32 * value){
  ...
  unsigned int sizes[] = {sizeof(u256), sizeof(u32)};
  for(int i = 1; i < 2; i++){
    if(array[i] != NULL)
